...
from tensorpack.compat import is_tfv2



def train_model(hdf5_dir, brains_idx_dir, view, modified_unet=True, batch_size=16, val_batch_size=32,
                lr=0.01, epochs=100, hor_flip=False, ver_flip=False, zoom_range=0.0, 
                save_dir = '../handson-machine-learning/chapter_14/se_net/BTS/save',
    ...
    # add callbacks    
    save_dir = os.path.join(save_dir, f'{view}_{os.path.basename(brains_idx_dir)[:5]}')
    ...
    # training the model
        model.fit(datagen_train, epochs=epochs, use_multiprocessing=multiprocessing, 
                        callbacks=callbacks, validation_data = datagen_val, verbose=1)
...
if __name__ == '__main__':
    ...
    train_model(cfg['hdf5_dir'], cfg['brains_idx_dir'], cfg['view'], cfg['modified_unet'], cfg['batch_size'], 
                ...
                cfg['save_data_dir'], cfg['start_chs'], cfg['levels'], cfg['multiprocessing'],
                cfg['load_model_dir'])
