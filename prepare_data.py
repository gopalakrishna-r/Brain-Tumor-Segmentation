...
def read_brain(brain_dir, mode='train', x0=42, x1=194, y0=29, y1=221, z0=2, z1=146):
    ...
    brain_name = (
        f'{os.path.basename(os.path.split(brain_dir)[0])}_'
        + os.path.basename(brain_dir)
    )
...
def create_table(dataset_dir, table_data_shape, save_dir, crop_coordinates, data_channels, k_fold=None):
    ...
    hdf5_file = tables.open_file(f'{save_dir}data.hdf5', mode='w')
    ...
    if k_fold:
        ...
        for fold in range(k_fold):
            ...
            np.save(f'{save_dir}/fold{fold}_idx.npy', train_idx)
